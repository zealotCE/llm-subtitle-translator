...
> 当前已新增 “Quality & Ops Improvements” 里程碑，聚焦运行日志、队列策略、缓存与测试覆盖等稳定性工作。
...
- `EVAL_OUTPUT_DIR`：评估样本输出目录（默认 `output/eval`）
- `EVAL_SAMPLE_RATE`：采样比例（默认 `1.0`）
...
- `WORKER_CONCURRENCY`：任务并发数（默认 `2`）
- `FFMPEG_CONCURRENCY`：音频抽取并发（默认 `1`）
- `MAX_ACTIVE_JOBS`：全局任务并发上限（默认 `2`）
